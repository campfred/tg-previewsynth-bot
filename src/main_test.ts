...
Deno.test("Link conversion only", (): void =>
{
	...
	assertEquals(FurAffinityConverter.convertLink(Link), Result)
})
...
Deno.test("Full chain conversion", async (): Promise<void> =>
{
	...
	const Result: URL = new URL("https://xfuraffinity.net/view/58904471/")
	assertEquals(await FurAffinityConverter.parseLink(Link, FurAffinityConverter.destinations[0].hostname), Result)
})
...
Deno.test("TikTok-specific test case", async (): Promise<void> =>
{
	...
	assertEquals(await Converter.parseLink(Link, Converter.destinations[0].hostname), Result)
})
...
Deno.test("YouTube-specific constraint", function (): void
{
	...
	assert(!Converter.isSupported(Link))
})
...
Deno.test("FurTrack index translation", async function (): Promise<void>
{
	...
	assertEquals(await FurTrackConverter.parseLink(NavLink, FurTrackConverter.destinations[0].hostname), ConvertedLink)
})
...
Deno.test("Music-specific test case", async (): Promise<void> =>
{
	...
	assertEquals(await Converter.parseLink(SpotifyShareLink, Converter.destinations[0].hostname), OdesliConvertedLink)
})
