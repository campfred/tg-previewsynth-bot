...
Deno.test("Full chain conversion", async (): Promise<void> =>
{
	...
	const Result: URL = new URL("https://xfuraffinity.net/view/58904471/")
	assertEquals(await FurAffinityConverter.parseLink(Link, FurAffinityConverter.defaultDestination), Result)
})
...
Deno.test("TikTok-specific test case", async (): Promise<void> =>
{
	...
	assertEquals(await Converter.parseLink(Link, Converter.defaultDestination), Result)
})
...
Deno.test("FurTrack index translation", async function (): Promise<void>
{
	...
	assertEquals(await FurTrackConverter.parseLink(NavLink, FurTrackConverter.defaultDestination), ConvertedLink)
})
...
Deno.test("Music-specific test case", async (): Promise<void> =>
{
	...
	assertEquals(await Converter.parseLink(SpotifyShareLink, Converter.defaultDestination), OdesliConvertedLink)
})
